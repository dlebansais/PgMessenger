...
namespace PgMessenger
{
    public partial class LoadAtStartupWindow : Window
    {
        ...
        public LoadAtStartupWindow()
        {
            ...
            try
            {
                string ApplicationFolder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "PgMessenger");

                if (!Directory.Exists(ApplicationFolder))
                    Directory.CreateDirectory(ApplicationFolder);

                TaskFile = Path.Combine(ApplicationFolder, "Project Gorgon - Messenger.xml");
                ...
                if (!File.Exists(TaskFile))
                {
                    ...
                    foreach (string ResourceName in ExecutingAssembly.GetManifestResourceNames())
                        if (ResourceName.EndsWith("PgMessenger.xml"))
                        {
