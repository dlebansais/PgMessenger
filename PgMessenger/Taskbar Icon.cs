...
{
    public interface ITaskbarClient
    {
        bool IsOpen { get; }
        IInputElement Input { get; }
        void Open();
        void SetForeground();
        void OnMenuOpened(ToolStripItemCollection Items, IReadOnlyDictionary<ToolStripMenuItem, ICommand> CommandTable);
        event EventHandler ClientClosed;
    }

    public class TaskbarIcon : IDisposable
    {
        #region Init
        protected TaskbarIcon(NotifyIcon NotifyIcon, ITaskbarClient Target)
        {
            this.NotifyIcon = NotifyIcon;
            this.Target = Target;

            LastClosedTime = DateTime.MinValue;
            Target.ClientClosed += OnClosed;
        }

        protected static List<TaskbarIcon> ActiveIconList { get; private set; } = new List<TaskbarIcon>();
        private NotifyIcon NotifyIcon;
        private ITaskbarClient Target;
        #endregion

        #region Client Interface
        public static TaskbarIcon Create(Icon Icon, string ToolTipText, System.Windows.Controls.ContextMenu Menu, ITaskbarClient Target)
        {
            try
            {
        ...
        private void OnMenuOpened(object sender, EventArgs e)
        {
            ...
            Target.OnMenuOpened(ContextMenuStrip.Items, CommandTable);
        }
        ...
        private ContextMenuStrip MenuToMenuStrip(System.Windows.Controls.ContextMenu Menu)
        {
            ...
            ConvertToolStripMenuItems(ContextMenuItems, Result.Items);
        ...
        private static void OnMenuClicked(ToolStripMenuItem MenuItem)
        {
            if (MenuTable.ContainsKey(MenuItem) && CommandTable.ContainsKey(MenuItem))
            {
                ...
                if (Command != null)
                    Command.Execute(TaskbarIcon, TaskbarIcon.Target.Input);
            }
        }

        private System.Windows.Controls.ItemCollection ContextMenuItems;
